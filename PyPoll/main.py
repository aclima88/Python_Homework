...
voteSummary_df = pd.DataFrame({'Candidate': votesPerCandidate,
                               
                               # Convert the convert the 'Vote Percentage' column to a string using .astype(str) and then add the % symbol to the end of the percentage number by concatenating the "%" symbol using the '+' operator and round the output to the third decimal place
                               ...
                               'Votes': '(' + votesPerCandidate.round(3).astype(str) + ')'})

# Locate the candidate with the most votes using the loc method from pandas
winner = voteSummary_df['Candidate'].idxmax()
